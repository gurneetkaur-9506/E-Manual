...
export const ImageLightbox: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-xl animate-in fade-in duration-200">
      ...
      <div className="relative w-full max-w-5xl rounded-2xl border border-cyan-500/30 bg-slate-900 shadow-2xl overflow-hidden flex flex-col max-h-[90vh] z-10">
        ...
        <div className="flex-1 overflow-auto p-6 bg-slate-950/80 flex items-center justify-center min-h-[350px]">
          <div
          ...
          >
            <img
              ...
              className="max-h-[65vh] w-auto object-contain rounded-lg shadow-2xl border border-slate-800 bg-slate-900"
            />
          </div>
        </div>

        {/* Lightbox Caption Footer */}
        <div className="p-4 border-t border-slate-800 bg-slate-950 text-xs text-slate-300 flex items-center justify-between">
          <div>
            <span className="font-semibold text-white mr-1.5">{lightboxFigure.caption}</span>
            {lightboxFigure.details && (
              <span className="text-slate-400">{lightboxFigure.details}</span>
            )}
          </div>

          {lightboxFigure.pageRef && (
            <span className="text-[11px] font-mono text-slate-500 bg-slate-900 px-2 py-0.5 rounded border border-slate-800 flex-shrink-0 ml-2">
